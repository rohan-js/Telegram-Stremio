...
class GeminiMatcherTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_low_confidence_close_candidates_call_gemini(self):
        async def fake_request(prompt, timeout):
            return {"selected_candidate_index": 1, "confidence": 88, "reason": "better release year"}, "gemini-test"
        ...
        self.assertEqual(result.reason, "accepted_gemini_rerank")
        self.assertTrue(result.extra["gemini_used"])
        self.assertEqual(result.extra["gemini_confidence"], 88)

    async def test_timeout_falls_back_quickly(self):
        async def slow_request(prompt, timeout):
            await asyncio.sleep(0.5)
            return {"selected_candidate_index": 1, "confidence": 88, "reason": "late"}, "gemini-test"
        ...
        self.assertTrue(result.extra["gemini_timeout"])

    async def test_invalid_json_shape_falls_back(self):
        async def fake_request(prompt, timeout):
            return {"selected_candidate_index": 99, "confidence": 50, "reason": "bad"}, "gemini-test"

        with patch.object(gemini_matcher, "_request_with_optional_fallback", side_effect=fake_request):
            result = await maybe_rerank_with_gemini(self._intent(), self._decision(), self._candidates())

        self.assertEqual(result.candidate.tmdb_id, 1)
        self.assertTrue(result.extra["gemini_invalid"])
    ...
    async def test_cache_prevents_duplicate_gemini_calls(self):
        ...
        async def fake_request(prompt, timeout):
            nonlocal calls
            calls += 1
            return {"selected_candidate_index": 1, "confidence": 90, "reason": "cached"}, "gemini-test"
        ...
        self.assertTrue(second.extra["gemini_cached"])
    ...
    async def test_wrong_media_type_selection_is_ignored(self):
        ...
        async def fake_request(prompt, timeout):
            return {"selected_candidate_index": 1, "confidence": 80, "reason": "wrong type"}, "gemini-test"
