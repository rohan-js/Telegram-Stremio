...
# config.env is dotenv-quoted (KEY="value"); docker --env-file keeps quotes
# literally, so emit a cleaned copy for the verification runs only.
verify_env="$(mktemp)"
python3 - "${APP_DIR}/config.env" "${verify_env}" <<'PY'
import sys
...
PY
trap 'rm -f "${verify_env}"' EXIT

built_version="$(
  docker run --rm --env-file "${verify_env}" --entrypoint python "${IMAGE}" -c \
