...
def _find_split_match(name: str) -> Optional[Tuple[int, int, int, Optional[str]]]:
    match = _TRAILING_NUMERIC_PATTERN.search(name)
    if match:
        return match.start(), match.end(), int(match.group(2)), match.group(1)

    match = _NUMERIC_PATTERN.search(name)
    if match:
        part_num = int(match.group(1))
        if 1 <= part_num <= 99:
            return match.start(), match.end(), part_num, None
