...
class Database:
    ...
    # -------------------------------
    # Token Management for Subscriptions
    # -------------------------------
    ...
    async def create_token(
        ...
        plan: str,
        days: int,
        daily_limit_gb: Optional[float] = None
    ) -> str:
        ...
        await self.dbs["tracking"]["tokens"].update_many(
            {"user_id": user_id, "active": True},
            {"$set": {"active": False}}
        )
        
        await self.dbs["tracking"]["tokens"].insert_one(token_doc)
        LOGGER.info(f"Created {plan} token for user {user_id}, expires {expiry}")
        return token

    async def get_token(self, token: str) -> Optional[Dict]:
        """Get token details and verify it's valid"""
        token_doc = await self.dbs["tracking"]["tokens"].find_one({
            "token": token,
            "active": True
        })
        ...
        if token_doc["usage"]["last_reset"] != today:
            await self.dbs["tracking"]["tokens"].update_one(
                {"token": token},
                {"$set": {
                    "usage.today": 0,
            ...
            token_doc["usage"]["today"] = 0
        
        return convert_objectid_to_str(token_doc)

    async def update_token_usage(self, token: str, bytes_used: int) -> bool:
        """Update usage stats for a token"""
        gb_used = bytes_used / (1024 ** 3)
        
        result = await self.dbs["tracking"]["tokens"].update_one(
    ...
    async def check_daily_limit(self, token: str) -> Tuple[bool, Optional[float]]:
        """Check if user has exceeded daily limit. Returns (within_limit, remaining_gb)"""
        token_doc = await self.get_token(token)
        if not token_doc:
            return False, None
        
        daily_limit = token_doc.get("daily_limit_gb")
        if not daily_limit:
            return True, None  # No limit
        
        used_today = token_doc["usage"]["today"]
        remaining = daily_limit - used_today
        
        return remaining > 0, max(0, remaining)

    async def get_token_by_user(self, user_id: int) -> Optional[Dict]:
        ...
        if token_doc and datetime.utcnow() > token_doc["expires_at"]:
            return None
            
        return convert_objectid_to_str(token_doc) if token_doc else None
    ...
    async def list_subscribers(self, active_only: bool = True) -> List[Dict]:
        ...
        cursor = self.dbs["tracking"]["tokens"].find(query).sort("created_at", -1)
        subscribers = await cursor.to_list(length=100)
        return [convert_objectid_to_str(s) for s in subscribers]

    async def get_subscription_stats(self) -> Dict:
        """Get subscription statistics"""
        total = await self.dbs["tracking"]["tokens"].count_documents({})
        active = await self.dbs["tracking"]["tokens"].count_documents({"active": True})
        ...
        plan_counts = await self.dbs["tracking"]["tokens"].aggregate(pipeline).to_list(None)
        
        return {
            "total_tokens": total,
            "active_tokens": active,
            "by_plan": {p["_id"]: p["count"] for p in plan_counts}
        }
