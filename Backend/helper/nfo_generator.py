"""Kodi-style NFO XML builders (ported from upstream weebzone/Telegram-Stremio).

Pure stdlib, no I/O: ``movie_nfo`` / ``tvshow_nfo`` / ``episode_nfo`` /
``season_nfo`` consume the media documents we already store in Mongo and
return NFO XML strings. Delivery is ours: admin download endpoints
(see fastapi/routes/api_routes.py) and optional .nfo writes next to
completed torrent downloads (see helper/torrent_downloads.py).
"""
...
def movie_nfo(doc: Dict[str, Any]) -> str:
    ...
    lines.append("  <source>Telegram-Stremio</source>")
    lines.append("</movie>")
    return "\n".join(lines) + "\n"
