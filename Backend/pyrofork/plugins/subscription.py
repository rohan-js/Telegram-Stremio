...
PLANS = {
    ...
    "basic": {"days": 30, "daily_limit_gb": 10, "price": 50},
}
...
async def start_command(client: Client, message: Message):
    ...
    if existing_token:
        await message.reply(
            ...
            f"✅ You have an active **{existing_token['plan'].title()}** subscription\n"
...
async def status_command(client: Client, message: Message):
    ...
    daily_limit = token_data.get("daily_limit_gb", 0)
    used_today = usage.get("today", 0)
    total_used = usage.get("total", 0)
    
    remaining = max(0, daily_limit - used_today) if daily_limit else "Unlimited"
    
    await message.reply(
        f"📊 **Subscription Status**\n\n"
        f"**Plan:** {token_data['plan'].title()}\n"
        f"**Expires:** {token_data['expires_at'][:10]}\n\n"
        f"**Today's Usage:**\n"
        f"• Used: {used_today:.2f} GB\n"
        f"• Remaining: {remaining if isinstance(remaining, str) else f'{remaining:.2f} GB'}\n"
        f"• Limit: {daily_limit} GB/day\n\n"
...
async def users_command(client: Client, message: Message):
    ...
    text = f"👥 **Active Subscribers: {stats['active_tokens']}**\n\n"
    ...
    for sub in subscribers[:20]:  # Limit to 20
        ...
        text += f"• `{user_id}` | {plan} | {expires} | {usage:.1f}GB today\n"
